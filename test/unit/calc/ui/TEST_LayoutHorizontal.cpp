...
/**************************** INTELLECTUAL PROPERTY RIGHTS ****************************/
/**
 * @file    TEST_LayoutHorizontal.cpp
 * @author  Marvin Smith
 * @date    03/25/2025
 *
...
class gui_LayoutHorizontal : public testing::Test 
{
    protected:
...
}; // End of gui_LayoutHorizontal class
 ...
 */
TEST_F( gui_LayoutHorizontal, allocate_bbox_test1 )
{
    ...
    // Set padding
    int x_top_pad = 100;
    layout->set_padding( x_top_pad, 100, 100, 100 );

    // Set the layout size
    layout->set_layout_size( math::Size2i( { 1000, 800 } ) );

    // Demo alignment policy
    std::set<gui::AlignmentPolicy> alignment { { gui::AlignmentPolicy::LEFT, 
                                                 gui::AlignmentPolicy::CENTER_VERTICAL } };

    // This gives us a working window of 800 x 500 starting at 100, 100
    ...
    int item1_width = static_cast<int>((0.6/1.4)*800);
    ASSERT_NEAR( bboxes[0].min().x(), 100, 0.001 );
    ASSERT_NEAR( bboxes[0].min().y(), 100, 0.001 );
    ASSERT_NEAR( bboxes[0].width(),   item1_width, 0.001 );
    ASSERT_NEAR( bboxes[0].height(),  600, 0.001 );

    // Verify item 2, which is 0.8, has (0.8/1.4)*800 space
    int item2_width = static_cast<int>((0.8/1.4)*800);
    ASSERT_NEAR( bboxes[1].min().x(), item1_width + x_top_pad, 0.001 );
    ASSERT_NEAR( bboxes[1].min().y(), 100, 0.001 );
    ASSERT_NEAR( bboxes[1].width(),   item2_width, 0.001 );
    ASSERT_NEAR( bboxes[1].height(),  600, 0.001 );
